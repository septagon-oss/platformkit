...
// AUTH_DENIED — the roles screen, the users screen and the tenant list at once —
// and the delete case left the next test unable to sign in at all. The fixture's
...
test('the last administrator cannot delete themselves off the user screen', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/admin\/user\/users$/);
  ...
  await page.goto(`/admin/user/users/${(await identity.json()).userId}`);
  ...
  await page.goto('/admin/user/users');
...
test('the last administrator cannot be stripped or deactivated through the commands', async ({ page }) => {
  ...
  expect((await page.request.get('/api/v1/tenant/tenants')).status()).toBe(200);
});
