// Package smtp delivers mail through an explicitly configured SMTP server.
package smtp
...
// Mail selects one outgoing server. Its shape remains compatible with the
// configuration supplied by existing PlatformKit composition.
type Mail struct {
	Host     string
	Port     int
	Username string
...
// SMTP implements mail.Mailer with stdlib net/smtp, STARTTLS when the server
// offers it and authentication only when a username is configured.
...
// New returns the sender for cfg without connecting to the server.
func New(cfg Mail) *SMTP { return &SMTP{cfg: cfg} }

var _ mail.Mailer = (*SMTP)(nil)

// Send delivers one message. The connection is opened, used and closed per
// message. This provider performs no retries: the caller owns its delivery
// policy. Context cancellation controls dialing; after connection, per-step
// deadlines bound a stalled conversation rather than the context deadline.
func (s *SMTP) Send(ctx context.Context, m mail.Message) error {
	...
	c, err := netsmtp.NewClient(paced{conn}, s.cfg.Host)
	if err != nil {
		_ = conn.Close()
	...
	if s.cfg.Username != "" {
		if err := c.Auth(netsmtp.PlainAuth("", s.cfg.Username, s.cfg.Password, s.cfg.Host)); err != nil {
...
func wire(from string, m mail.Message) string {
