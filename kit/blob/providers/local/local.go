// Package local stores blobs in an explicitly supplied filesystem directory.
package local
...
// Local implements blob.Storage and blob.Lister on the filesystem.
//
// A key is a lowercase UUID supplied by the caller and checked here. That is the
...
// Keys is blob.Lister on the filesystem: every blob written before before.
//
