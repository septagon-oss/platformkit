...
func RunStorage(t *testing.T, fresh func(*testing.T) StorageFixture) {
	...
	t.Run("independent scopes and optional listing cutoff", func(t *testing.T) {
		...
		for i, store := range []contracts.Storage{first, second} {
			if lister, ok := store.(contracts.Lister); ok {
...
func storagePut(t *testing.T, store contracts.Storage, key, body string) {
	t.Helper()
...
func storageRead(t *testing.T, store contracts.Storage, key string) string {
	t.Helper()
...
func storageMissing(t *testing.T, store contracts.Storage, key string) {
	t.Helper()
	body, err := store.Get(t.Context(), key)
	if body != nil {
		_ = body.Close()
