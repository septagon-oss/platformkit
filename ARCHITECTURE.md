...
| Native discovery | [`screens.Describe`](ui/screens/catalog.go) exposes `/api/v1/admin/resources`, stamped with `catalogVersion`; the native consumer owns its renderer. Each entry and each command carries JSON Schema 2020-12 for its fields beside the field list, from [`entity.JSONSchema`](kit/entity/jsonschema.go), and `kit/app.Describe` carries the same projection per resource and command. |
