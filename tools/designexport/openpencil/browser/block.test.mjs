...
test('source block flow retains intrinsic row sizing, linked paragraphs, property history and two saves', async t => {
  const directory = await mkdtemp(join(tmpdir(), 'platformkit-block-source-'))
  t.after(() => rm(directory, { recursive: true, force: true }))
  const file = join(directory, 'main.go'), id = 'fixture/block'
...
`, { flag: 'wx' })
  const source = (content = 'An album begins here.', row = false) => JSON.parse(execFileSync('go', ['run', file], {
    cwd: new URL('../../../../', import.meta.url), encoding: 'utf8', input: JSON.stringify({ content, row }),
  }))
