...
test('fixed absolute compositions retain linked content, edge placement, resizing and two saves', async t => {
...
func owner(_ struct{}, children ...g.Node) g.Node {
  return h.A(h.Href("/albums/fixture/7"), g.Attr("aria-label", "Open album"),
    h.Style("position:relative;display:flex;height:160px;gap:16px;align-items:center;padding:12px;border:solid transparent;border-width:2px 5px 4px 3px"), g.Group(children))
}
func main() {
  ...
  examples := []components.Example{components.ExampleWithChildren(components.ExampleInfo{
    ID:"fixture/positioning", ComponentID:"fixture.component.positioning"}, struct{}{}, children, owner)}
...
`)
  const id = 'fixture/positioning', snapshot = run({}), fonts = suppliedFonts([400])
  ...
  function matches(graph, observed) {
    ...
    assert.equal(placed.height, 160, 'absolute children do not contribute to normal-flow size')
  }
  try {
    for (const mode of ['light', 'dark']) {
      ...
      for (let cycle = 0; cycle < 3; cycle++) {
        ...
        const caption = graph.getChildren(badge.id).find(node => origin(node)?.localId === 'caption')
        const definition = chain(graph, caption, 'componentId').at(-1).componentPropertyDefinitions.find(item => item.name === 'content')
        editor.setInstanceComponentProperty(caption.id, definition.id, '99')
        const result = extractSourceProps(graph, caption, snapshot)
        assert.equal(result.status, 'proposal')
        assert.deepEqual(result.proposal.path, [id, 'corner3', 'caption'])
